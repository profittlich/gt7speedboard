#include "sb/components/MapRenderers/SBGLMapWidgetZoomedLines.h"
#include "sb/components/Map.h"


void SBGLMapWidgetZoomedLines::clearRefLap()
{
    m_verticesRef.clear();
    recalcExtents();
}
...
void SBGLMapWidgetZoomedLines::addPoint(const PTelemetryPoint & p)
{
    ...
    m_minX = p->position().x() - 100;
    m_maxX = p->position().x() + 100;
    m_minY = p->position().z() - 100;
    m_maxY = p->position().z() + 100;
...
void SBGLMapWidgetZoomedLines::nextLap()
{
...
void SBGLMapWidgetZoomedLines::initializeGL()
{
...
void SBGLMapWidgetZoomedLines::resizeGL(int w, int h)
{
    if (w > h)
    {
...
void SBGLMapWidgetZoomedLines::paintGL()
{
    if (!m_initialized)
    {
        return;
